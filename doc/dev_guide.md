...
### 1.1 产物文件要求
Obsidian 加载一个社区插件需要以下三个核心文件，存放在 `[YourVault]/.obsidian/plugins/obsidian-genwiki/` 目录下：
...
* **方式 A（软链接 - 推荐）**：
  ...
  ```bash
  ln -s /path/to/genwiki /path/to/GenWikiDevVault/.obsidian/plugins/obsidian-genwiki
  ```
* **方式 B（构建脚本重定向）**：
  修改 `esbuild.config.mjs` 中的 `outfile` 路径，直接指向 `GenWikiDevVault/.obsidian/plugins/obsidian-genwiki/main.js`。
